...
el idea de obtiene de areas en la primera posición y el id es el que se agrega
ejemplo: area -> [0] -> id
POST 
data:{
  title,
...
const saveIncidence = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      console.error('Error al guardar la incidencia');
    }
  } catch (error) {
    console.error('Error al intentar guardar la incidencia:', error);
  }
}
